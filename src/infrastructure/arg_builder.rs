...
pub fn build_command(
    ...
    match format {
        Format::VideoHighest => {
            // Best video + audio, remux to MP4 — avoids compound extensions like name.webm.mp4
...
mod tests {
    ...
    // 3.1 RED → GREEN — --no-playlist present for VideoHighest
    #[test]
    fn no_playlist_present_for_video_highest() {
        let url = make_url("https://example.com/video");
        let args = super::build_args(Format::VideoHighest, &url, "yt-dlp", "%(title)s.%(ext)s");
    ...
    // 3.5 RED → GREEN — VideoHighest includes --merge-output-format mp4
    #[test]
    fn video_highest_includes_mp4_output_format() {
        ...
        assert!(
            ...
            "VideoHighest must contain --merge-output-format"
        );
    ...
    fn default_binary_is_yt_dlp() {
        let url = make_url("https://example.com/video");
        let (binary, _args) =
            super::build_command(Format::VideoHighest, &url, "yt-dlp", "%(title)s.%(ext)s");
        assert_eq!(binary, "yt-dlp");
    }

    // 3.8 RED → GREEN — VideoHighest and AudioMp3 produce distinct flag sets
    #[test]
    fn video_highest_and_audio_mp3_produce_distinct_flag_sets() {
        let url = make_url("https://example.com/video");
        let video_args =
            super::build_args(Format::VideoHighest, &url, "yt-dlp", "%(title)s.%(ext)s");
        let audio_args =
            super::build_args(Format::AudioMp3, &url, "yt-dlp", "%(title)s.%(ext)s");
        assert_ne!(
            video_args, audio_args,
            "VideoHighest and AudioMp3 must produce distinct argument vectors"
        );
    }
}
